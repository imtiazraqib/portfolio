    ...
    <Flex fillWidth gap="m" direction="column">
      {images[activeIndex] && (
        <Flex onClick={handleImageClick}>
          <RevealFx style={{ width: "100%" }} delay={0.4} trigger={isTransitioning} speed="fast">
            <SmartImage
              tabIndex={0}
              radius="l"
              alt={title}
      ...
      <Flex mobileDirection="column" fillWidth paddingX="s" paddingTop="12" paddingBottom="24" gap="l">
        ...
        {(avatars?.length > 0 || description?.trim() || content?.trim()) && (
          <Flex flex={7} direction="column" gap="16">
            {avatars?.length > 0 && <AvatarGroup avatars={avatars} size="m" reverseOrder />}
            {description?.trim() && (
            ...
            {content?.trim() && (
              <SmartLink suffixIcon="chevronRight" style={{ margin: "0", width: "fit-content" }} href={href}>
                <Text variant="body-default-s">{t("projectCard.label")} Test</Text>
              </SmartLink>
            )}
          </Flex>
        )}
